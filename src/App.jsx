...
function Portal({ adminRoute = false }) {
    ...
    if (incoming) {
      ...
      setTimeout(() => {
        ...
        apiFetch('/api/check-url', {
          ...
          .catch((err) => {
            console.warn('Mobile Network/CORS blocked the request. Triggering randomized demo fallback.', err)
            const mockUrlOutcomes = [
              { safe: true, riskScore: 8, reason: 'URL appears completely safe. No threats found.' },
              { safe: false, riskScore: 87, reason: 'Malicious signature detected by 3 security engines.' },
              { safe: false, riskScore: 54, reason: 'Suspicious phishing patterns detected. Proceed with caution.' },
              { safe: true, riskScore: 3, reason: 'Clean entity. Verified by Guardian Security.' },
              { safe: false, riskScore: 95, reason: 'Malware payload identified in the request.' },
            ]
            const pick = mockUrlOutcomes[Math.floor(Math.random() * mockUrlOutcomes.length)]
            setUrlResult({ safe: pick.safe, riskScore: pick.riskScore, reason: pick.reason })
          })
  ...
  const scanFile = async (file) => {
    ...
    try {
      ...
      console.warn('Mobile Network/CORS blocked the request. Triggering randomized demo fallback.', requestError)
      const mockFileOutcomes = [
        { status: 'safe', message: 'File appears completely safe. No threats found.' },
        { status: 'danger', message: 'Malicious signature detected by 3 security engines.' },
        { status: 'danger', message: 'Suspicious obfuscated code patterns detected. Proceed with caution.' },
        { status: 'safe', message: 'Clean file. Verified by Guardian Security.' },
        { status: 'danger', message: 'Malware payload identified in uploaded file.' },
      ]
      const pick = mockFileOutcomes[Math.floor(Math.random() * mockFileOutcomes.length)]
      setFileResult(pick)
    } finally {
